document.addEventListener('DOMContentLoaded', () => {
  ...
  const defaultTemplates = {
    dashboard: `
      ...
      <p>Este relatório inclui gráficos e estatísticas da sua coleção de filmes e pessoas.</p>
    `,
    summary: `
  ...
  function saveTemplate() {
    ...
    let templates = JSON.parse(localStorage.getItem('responsiveTemplates') || '[]');
    templates.push(templateData);
    localStorage.setItem('responsiveTemplates', JSON.stringify(templates));
  ...
  function showLoadModal() {
    const templates = JSON.parse(localStorage.getItem('responsiveTemplates') || '[]');
    ...
    if (template) {
      ...
      template.charts.forEach(chartData => {
        if (chartData) {
          const chartItem = document.createElement('div');
          chartItem.className = 'chart-item';
          chartItem.innerHTML = `<img src="${chartData}" style="width:100%;" />`;
          document.getElementById('charts-container').appendChild(chartItem);
        }
      });
